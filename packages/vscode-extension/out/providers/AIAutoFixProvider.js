...
exports.AIAutoFixProvider = void 0;
const vscode = __importStar(require("vscode"));
const core_1 = require("@baseline-buddy/core");
class AIAutoFixProvider {
    constructor() {
        this.fixEngine = new core_1.AIAutoFixEngine();
    }
    ...
    createAutoFixAction(document, range, diagnostic) {
    ...
    createExplainAction(document, range, diagnostic) {
        const action = new vscode.CodeAction('💡 Explain Issue', vscode.CodeActionKind.Empty);
        action.command = {
    ...
    async executeAutoFix(document, range, diagnostic) {
        try {
            // Show progress
            await vscode.window.withProgress({
                ...
                if (success) {
                    ...
                    // Show notification
                    vscode.window.showInformationMessage(`✅ AI Auto-Fix Applied: ${bestFix.originalCode} → ${bestFix.fixedCode}`, 'View Details').then(selection => {
    ...
    async executeExplainIssue(document, range, diagnostic) {
        try {
            // Create fix context
            const context = {
                filePath: document.fileName,
    ...
    showFixDetails(fix) {
        const panel = vscode.window.createWebviewPanel('baselineFixDetails', '🤖 AI Auto-Fix Details', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    detectProjectType(document) {
        ...
        try {
            ...
            if (document.fileName.includes('react'))
                return 'react';
            if (document.fileName.includes('vue'))
                return 'vue';
            if (document.fileName.includes('angular'))
                return 'angular';
            return 'vanilla';
        }
        catch {
    ...
    extractIssueFromDiagnostic(diagnostic) {
        const message = diagnostic.message.toLowerCase();
        if (message.includes('auto-phrase'))
            return 'word-break: auto-phrase';
        if (message.includes('subgrid'))
            return 'subgrid';
        if (message.includes('container'))
            return '@container';
        if (message.includes('has'))
            return ':has()';
        if (message.includes('nesting'))
            return 'css-nesting';
    ...
    clearDiagnosticsForRange(document, range) {
...
exports.AIAutoFixProvider = AIAutoFixProvider;
exports.default = AIAutoFixProvider;
//# sourceMappingURL=AIAutoFixProvider.js.map