...
import { BaselineIssueProvider } from './providers/BaselineIssueProvider';
import { BaselineHoverProvider } from './providers/BaselineHoverProvider';
import { BaselineCodeActionProvider } from './providers/BaselineCodeActionProvider';
import { AIAutoFixProvider } from './providers/AIAutoFixProvider';
import { BaselineStatusBar } from './statusBar/BaselineStatusBar';
import { BaselineWebviewProvider } from './webview/BaselineWebviewProvider';

let baselineDetector: WorkingBaselineDetector;
let featureAnalyzer: WorkingFeatureAnalyzer;
let issueProvider: BaselineIssueProvider;
let hoverProvider: BaselineHoverProvider;
let autoFixProvider: AIAutoFixProvider;
let codeActionProvider: BaselineCodeActionProvider;
let statusBar: BaselineStatusBar;
let webviewProvider: BaselineWebviewProvider;

export function activate(context: vscode.ExtensionContext) {
    console.log('Baseline Buddy extension is now active!');

    // Initialize core components
    baselineDetector = new WorkingBaselineDetector();
    featureAnalyzer = new WorkingFeatureAnalyzer();

  // Initialize providers
  issueProvider = new BaselineIssueProvider(baselineDetector, featureAnalyzer);
  hoverProvider = new BaselineHoverProvider(baselineDetector);
  autoFixProvider = new AIAutoFixProvider();
  codeActionProvider = new BaselineCodeActionProvider(baselineDetector);
  statusBar = new BaselineStatusBar();
  webviewProvider = new BaselineWebviewProvider(context);
    ...
    supportedLanguages.forEach(lang => {
        context.subscriptions.push(
            vscode.languages.registerCodeActionsProvider(lang, autoFixProvider)
        );
    });
...
function registerCommands(context: vscode.ExtensionContext, diagnosticCollection: vscode.DiagnosticCollection) {
    ...
    context.subscriptions.push(
        ...
        aiAutoFixCommand,
        explainIssueCommand,
        showFeatureInfoCommand
    );
}

function registerViews(context: vscode.ExtensionContext) {
    // Register tree data provider for issues
    const issuesProvider = new BaselineIssueProvider(baselineDetector, featureAnalyzer);
...
async function scanWorkspace(diagnosticCollection: vscode.DiagnosticCollection) {
    const config = vscode.workspace.getConfiguration('baseline-buddy');
    const includePatterns = config.get<string[]>('includePatterns', ['**/*.css', '**/*.js', '**/*.jsx', '**/*.ts', '**/*.tsx', '**/*.html']);
...
async function scanFile(uri: vscode.Uri, diagnosticCollection: vscode.DiagnosticCollection): Promise<vscode.Diagnostic[]> {
    try {
        ...
        const diagnostics: vscode.Diagnostic[] = issues.map(issue => {
            const range = new vscode.Range(
                ...
                0,
                issue.line ? issue.line - 1 : 0,
                document.lineAt(issue.line ? issue.line - 1 : 0).text.length
            );
...
async function autoFixFile(editor: vscode.TextEditor, diagnosticCollection: vscode.DiagnosticCollection) {
...
async function showFeatureInfo(feature: string) {
    try {
        const info = await baselineDetector.getFeatureInfo(feature);
        if (info) {
            const panel = vscode.window.createWebviewPanel(
                'baselineFeatureInfo',
                `Baseline Info: ${feature}`,
...
// Helper classes
class BaselineFeatureProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
