...
export class BaselineHoverProvider implements vscode.HoverProvider {
    ...
    async provideHover(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Promise<vscode.Hover | undefined> {
        ...
        if (!config.get<boolean>('showHoverInfo', true)) {
            return undefined;
        }
    ...
    private async getBaselineInfo(property: string, value: string, lineText: string): Promise<vscode.MarkdownString | undefined> {
        try {
            // Check for CSS Grid
    ...
    private createHoverContent(info: any, message: string): vscode.MarkdownString {
        ...
        if (info.support.chrome) content.appendMarkdown(`- Chrome: ${info.support.chrome}\n`);
        if (info.support.firefox) content.appendMarkdown(`- Firefox: ${info.support.firefox}\n`);
        if (info.support.safari) content.appendMarkdown(`- Safari: ${info.support.safari}\n`);
        if (info.support.edge) content.appendMarkdown(`- Edge: ${info.support.edge}\n`);
