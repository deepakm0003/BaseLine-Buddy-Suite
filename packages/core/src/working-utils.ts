...
export function formatFilePath(filePath: string, basePath?: string): string {
  if (basePath) {
...
export function formatBaselineStatus(baseline: string | false): string {
  switch (baseline) {
    case 'widely':
...
export function generateReportSummary(result: WorkingScanResult): string {
...
export function formatIssueDetails(issue: WorkingBaselineIssue, index: number): string {
...
export function generateHTMLReport(result: WorkingScanResult): string {
...
export function generateJSONReport(result: WorkingScanResult): string {
  return JSON.stringify(result, null, 2);
}

export function validateBaselineStatus(status: string | false): boolean {
  return ['limited', 'newly', 'widely', false].includes(status);
}

export function sortIssuesBySeverity(issues: WorkingBaselineIssue[]): WorkingBaselineIssue[] {
  const severityOrder = { error: 0, warning: 1, info: 2 };
  return issues.sort((a, b) => severityOrder[a.severity] - severityOrder[b.severity]);
}

export function filterIssuesBySeverity(issues: WorkingBaselineIssue[], severity: string): WorkingBaselineIssue[] {
  return issues.filter(issue => issue.severity === severity);
}

export function groupIssuesByFile(issues: WorkingBaselineIssue[]): { [file: string]: WorkingBaselineIssue[] } {
  return issues.reduce((groups, issue) => {
    ...
    return groups;
  }, {} as { [file: string]: WorkingBaselineIssue[] });
}
