/**
 * Baseline Buddy Core - Core Baseline detection and analysis logic
 *
 * This module provides the foundational functionality for detecting
 * and analyzing web features against Baseline status.
 */

// Working implementations
export * from './working-baseline-detector';
export * from './working-feature-analyzer';
export * from './working-ai-suggestions';
export * from './working-utils';

// AI Auto-Fix Engine
export * from './ai-auto-fix-engine';

